...
def view_books():
    ...
    for i, book in enumerate(library, start=1):
        ...
        print(
            f"{i}. {book['title']} by {book['author']} published in {book['year_of_publication']} "
            f"with ISBN {book['isbn']}. Available? {availability}"
        )
...
def search_book(title):
    for book in library:
        if book["title"] == title:
            availability = "Yes" if book["available"] else "No"
            print(
                f"Book found: {book['title']} by {book['author']} ({book['year_of_publication']}) | "
                f"ISBN: {book['isbn']} | Available? {availability}"
            )
            return
    ...
    for book in library:
        if book["isbn"] == isbn:
            if book["available"]:
                ...
                print("This book has already been borrowed. You may check back later.")
            return
    print("Book not found")
...
while True:
    ...
    if choice == "1":
        ...
        isbn = input("Enter the ISBN of the book to be updated: ").strip()
        update_book(isbn)

    elif choice == "4":
        isbn = input("Enter the ISBN of the book to be deleted: ").strip()
        delete_book(isbn)
